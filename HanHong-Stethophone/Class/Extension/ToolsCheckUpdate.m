...
- (void)actionToCheckUpdate:(Boolean)bShowToast{
    ...
    [TTRequestManager appsRequirements:params success:^(id  _Nonnull responseObject) {
...
- (void)actionSuccessCallback:(id)responseObject bShowToast:(Boolean)bShowToast{
    NSDictionary *iOSData = responseObject[@"data"][@"ios"];
    NSString * currentVersion = [NSBundle mainBundle].infoDictionary[@"CFBundleVersion"];
    NSInteger versionCodeIinimum = [iOSData[@"version_code_minimum"] integerValue];
    NSInteger versionCodeLatest = [iOSData[@"version_code_latest"] integerValue];
    if ([currentVersion integerValue] < versionCodeIinimum) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"needUpdateApp"];
        [Tools showAlertView:[NSString stringWithFormat:@"发现新版本%@",iOSData[@"version_name_latest"]] andMessage:@"" andTitles:@[ @"去更新"] andColors:@[MainColor] sure:^{
        ...
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"needUpdateApp"];
