...
#ifdef DEBUG
        DDLogDebug(@"最小时间date：%@", self.minLimitDate);
        DDLogDebug(@"默认时间date：%@", self.selectDate);
        DDLogDebug(@"最大时间date：%@", self.maxLimitDate);
        
        DDLogDebug(@"最小时间：%@", [NSDate br_getDateString:self.minLimitDate format:self.selectDateFormatter]);
        DDLogDebug(@"默认时间：%@", [NSDate br_getDateString:self.selectDate format:self.selectDateFormatter]);
        DDLogDebug(@"最大时间：%@", [NSDate br_getDateString:self.maxLimitDate format:self.selectDateFormatter]);
#endif
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    // 设置分割线的颜色
    ((UIView *)[pickerView.subviews objectAtIndex:1]).backgroundColor = [UIColor colorWithRed:195/255.0 green:195/255.0 blue:195/255.0 alpha:0.4f];
