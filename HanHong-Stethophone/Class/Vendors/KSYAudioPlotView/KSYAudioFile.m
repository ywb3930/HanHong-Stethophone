...
{
    return [self initWithURL:url delegate:delegate clientFormat:[self.class defaultClientFormat]];
}
...
{
    return [[self alloc] initWithURL:url delegate:delegate clientFormat:clientFormat];
}
...
- (BOOL)openAudioFile
{
    ...
    if (fileExists)
    {
        [KSYAudioUtilities checkResult:ExtAudioFileOpenURL(url, &self.info->extAudioFileRef) operation:"Failed to create ExtAudioFileRef"];
    }
    else
    {
        return NO;
    ...
    [KSYAudioUtilities checkResult:ExtAudioFileGetProperty(self.info->extAudioFileRef, kExtAudioFileProperty_AudioFile, &propSize, &self.info->audioFileID) operation:"Failed to get underlying AudioFileID"];
    
    //
    // Store the file format
    //
    propSize = sizeof(self.info->fileFormat);
    [KSYAudioUtilities checkResult:ExtAudioFileGetProperty(self.info->extAudioFileRef, kExtAudioFileProperty_FileDataFormat, &propSize, &self.info->fileFormat) operation:"Failed to get file audio format on existing audio file"];
    
    //
    // Get the total frames and duration
    //
    propSize = sizeof(SInt64);
    [KSYAudioUtilities checkResult:ExtAudioFileGetProperty(self.info->extAudioFileRef, kExtAudioFileProperty_FileLengthFrames, &propSize, &self.info->frames) operation:"Failed to get total frames"];
...
- (void)seekToFrame:(SInt64)frame
{
    ...
    {
        [KSYAudioUtilities checkResult:ExtAudioFileSeek(self.info->extAudioFileRef, frame) operation:"Failed to seek frame position within audio file"];
...
- (KSYAudioFloatData *)getWaveformDataWithNumberOfPoints:(UInt32)numberOfPoints
{
    ...
    if (pthread_mutex_trylock(&_lock) == 0)
    {
        ...
        // seek to 0
        [KSYAudioUtilities checkResult:ExtAudioFileSeek(self.info->extAudioFileRef, 0) operation:"Failed to seek frame position within audio file"];
        ...
        AudioBufferList *audioBufferList = [KSYAudioUtilities audioBufferListWithNumberOfFrames:(UInt32)framesPerBuffer numberOfChannels:self.info->clientFormat.mChannelsPerFrame interleaved:interleaved];
        ...
        for (SInt64 i = 0; i < numberOfPoints; i++)
        {
            ...
            [KSYAudioUtilities checkResult:ExtAudioFileRead(self.info->extAudioFileRef, &bufferSize, audioBufferList) operation:"Failed to read audio data from file waveform"];
            if (interleaved)
            {
            ...
            else
            {
                for (int channel = 0; channel < channels; channel++)
                {
                    ...
                    //NSLog(@"channel = %li, i = %i, rms = %f", channel, i, rms);
                    data[channel][i] = rms;
                }
            }
        }
        ...
        [KSYAudioUtilities checkResult:ExtAudioFileSeek(self.info->extAudioFileRef, currentFrame) operation:"Failed to seek frame position within audio file"];
        
        pthread_mutex_unlock(&_lock);
        
        waveformData = [KSYAudioFloatData dataWithNumberOfChannels:channels buffers:(float **)data bufferSize:numberOfPoints];
...
{
    [self getWaveformDataWithNumberOfPoints:KSYAudioFileWaveformDefaultResolution completion:waveformDataCompletionBlock];
}
...
{
    return [KSYAudioUtilities MAP:(float)[self frameIndex] leftMin:0.0f leftMax:(float)[self totalFrames] rightMin:0.0f rightMax:[self duration]];
}
...
{
    SInt64 frameIndex;
    [KSYAudioUtilities checkResult:ExtAudioFileTell(self.info->extAudioFileRef, &frameIndex) operation:"Failed to get frame index"];
    return frameIndex;
}
...
- (NSDictionary *)metadata
{
    ...
    UInt32          writable;
    [KSYAudioUtilities checkResult:AudioFileGetPropertyInfo(self.info->audioFileID, kAudioFilePropertyInfoDictionary, &propSize, &writable) operation:"Failed to get the size of the metadata dictionary"];
    
    // pull metadata
    [KSYAudioUtilities checkResult:AudioFileGetProperty(self.info->audioFileID, kAudioFilePropertyInfoDictionary, &propSize, &metadata) operation:"Failed to get metadata dictionary"];
...
- (void)setClientFormat:(AudioStreamBasicDescription)clientFormat
{
    ...
    //
    [KSYAudioUtilities checkResult:ExtAudioFileSetProperty(self.info->extAudioFileRef, kExtAudioFileProperty_ClientDataFormat, sizeof(clientFormat), &clientFormat) operation:"Couldn't set client data format on file"];
    ...
    self.floatData = [KSYAudioUtilities floatBuffersWithNumberOfFrames:1024 numberOfChannels:self.clientFormat.mChannelsPerFrame];
}
...
- (void)setCurrentTime:(NSTimeInterval)currentTime
{
    ...
    SInt64 frame = [KSYAudioUtilities MAP:currentTime leftMin:0.0f leftMax:[self duration] rightMin:0.0f rightMax:[self totalFrames]];
...
+ (instancetype)dataWithNumberOfChannels:(int)numberOfChannels buffers:(float **)buffers bufferSize:(UInt32)bufferSize
{
    ...
    float **buffersCopy = [KSYAudioUtilities floatBuffersWithNumberOfFrames:bufferSize numberOfChannels:numberOfChannels];
