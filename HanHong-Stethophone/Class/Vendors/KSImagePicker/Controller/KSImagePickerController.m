...
- (void)viewDidLoad {
    ...
    self.title = self.mediaType == KSImagePickerMediaTypePicture ? @"选择图片" : @"选择发布视频";
...
- (UIButton *)btnRight{
    if (!_btnRight) {
        ...
        [_btnRight addTarget:self action:@selector(nextState:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _btnRight;
}
...
- (void)nextState:(UIButton *)btn{
    if(!_selectedAsset ||  _selectedAsset.count == 0) {
        [self.view makeToast:@"你还没选择相关内容" duration:showToastViewErrorTime position:CSToastPositionCenter];
        return;
    }
...
//            publishView.selectedAsset = _selectedAsset;
...
- (void)loadView {
    ...
    //collectionView.frame = CGRectMake(0, kNavBarAndStatusBarHeight, screenW, screenH - kNavBarAndStatusBarHeight);
    self.view = view;
}
...
- (void)openSystemCamera {
    ...
    if (_mediaType == KSImagePickerMediaTypePicture) {
        [KSImagePickerController authorityCheckUpWithController:self type:KSImagePickerMediaTypeVideo completionHandler:^(KSImagePickerMediaType type) {
            ...
            //[weakSelf.navigationController pushViewController:cameraCtl animated:YES];
