...
- (void)setArrayStr:(NSArray *)arrayStr{
    _messageLabel.numberOfLines=0;
    _messageLabel.font = _messageFont;
    
    _messageLabel.textColor = MainGray;
    NSMutableAttributedString* atext=[[NSMutableAttributedString alloc]initWithString:_messageString];
    atext.yy_font = _messageFont;
    
    atext.yy_lineSpacing = 5.f;
    for (NSInteger i = 0; i < arrayStr.count; i++) {
        NSString *string = arrayStr[i];
        NSRange range0=[[atext string] rangeOfString:string];
        [atext yy_setTextHighlightRange:range0 color:MainColor backgroundColor:nil tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
            if(self.selectBlock){
                self.selectBlock(i);
            }
        }];
    ...
    _messageLabel.attributedText = atext;
}
...
- (YYLabel *)messageLabel
{
    if (!_messageLabel) {
        ...
        _messageLabel.preferredMaxLayoutWidth = viewWith-viewLeftRight*2;
...
- (void)addConstraintsAndActions
{
    ...
    }else
    {
        ...
        self.messageLabel.frame = CGRectMake(viewLeftRight, self.titleLabel.bottom+22, viewWith-viewLeftRight*2, [self.messageString getSpaceLabelHeightwithSpeace:messageLineSpace withLabel:self.messageLabel andAlignment:self.messageAlignment andWidth:viewWith-viewLeftRight*2 - 30]);
